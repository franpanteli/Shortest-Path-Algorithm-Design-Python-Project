...
"""
    Performing the algorithm multiple times in the same function:
        ...
        -> `targets_to_print`:
            ...
            -> The user can then specify what the target node will be <- if we want to get from the source node to another node 
            -> If they don't specify the target node, then the function will execute the algorithm between the start (source) 
        ...
        -> We also have a list of target nodes we want to execute the algorithm for 
        -> We are now taking this list of target nodes and executing the algorithm on them 
        -> So it is executed multiple times
        -> Each time we do this, we are calculating the shortest distance between the source and target nodes
        -> This returns the shortest distance between the nodes, and the path 
        -> That, times the number of different target nodes which we are having processed  
"""
